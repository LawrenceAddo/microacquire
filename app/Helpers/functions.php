...
if (!function_exists('getEmptyAvatarSvg')) {
  function getEmptyAvatarSvg($ind, $full_name = '', $shape = 0) {
    ...
    $colors = getGradients();
    // $ind = rand(0, count($colors) - 1);
    $ind = intval($ind % getGradientsCount());
    // $ind = max(0, min($ind, count($colors)-1));

    $col1 = $colors[$ind]['colors'][0];
    $col2 = $colors[$ind]['colors'][1];
    
    $shapeHtml = '<circle cx="100" cy="100" r="100" fill="url(#gradient)"/>';
    if ($shape == 0) {
      $shapeHtml = '<rect x="0" y="0" width="200" height="200" fill="url(#gradient)"/>';
    }
  ...
  ' . $shapeHtml . '
  <text x="50%" y="50%" textLength="180px" fill="#fff" text-anchor="middle" dy=".3em" lengthAdjust="spacingAndGlyphs" style="font-family: arial; font-weight: bold; font-size: 54">' . ucwords($full_name) . '</text>
  </svg>';
